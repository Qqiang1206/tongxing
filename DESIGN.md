...
- **Logo 尺寸统一（二次修正）**：用户反馈"大大小小不一致"。根因是 `object-fit: contain`
  ...
  解法：把所有 logo 预渲染到**统一画布 720×150**，字标占中间 100 单位高度（字高全站一致），
  **紧凑/堆叠型 logo（宽高比 < 1.8，如 HUAKETEK）允许占满 150** 做光学补偿。
  画布尺寸一致 ⇒ `contain` 在任何容器里都等比渲染。
...
- **砖砌咬合错落（2026-09-12）**：用户要求 logo 墙"错落、左右咬合，像八卦"。
  实现为**纯 CSS**（无需 JS 分行）：
  - 桌面 5 列时 13 个 logo 由 flex 自然换行成 5/5/3；用 CSS **`translate` 属性**
    （非 `transform`）给奇数行 `-27%`、偶数行 `+27%`，相邻行错开半格。
    `translate` 不影响 flex 换行，也不会与 hover 的 `transform: translateY` 冲突。
  - `nth-child(10n+1..5)` / `nth-child(10n+6..0)` 命中行序，**对任意数量都生效**
    （后台增删 logo 不需改代码）。
  - 仅在 `@media (min-width: 1024px)` 生效；平板 3 列 / 移动 2 列保持整齐网格。
  - 位移约 ±60px，落在 `.v3-shell` 内边距内（1440 下 72px），不裁切；首页该 section
    `py-14 → py-20` 留呼吸。
  - 已知取舍：末行不足 5 个时随奇数行左移约 5%（视觉上仍接近居中）。
- **关于页客户墙与首页统一（2026-09-12）**：去掉白卡片（`bg-white`/描边/圆角/阴影），
  改为悬浮式彩色 logo（与首页一致），hover 轻微上浮；错落规则与首页共用。
